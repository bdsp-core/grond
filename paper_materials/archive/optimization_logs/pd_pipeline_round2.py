...
def plot_eeg_traces(ax, eeg_data, title, discharge_times=None,
                    highlight_left=False, spacing=100.0): # Increased base spacing
    ...
    for idx in DISPLAY_ORDER:
        if idx == -1:
            # Significantly increased vertical spacing between channel groups
            y_pos -= spacing * 1.5
            continue
        # Increased vertical amplitude of EEG traces for better visibility
        trace = eeg_data[idx] * 2.0
    ...
    ax.set_xlim(0, 10)
    y_top = spacing * 0.8 # Adjusted top limit to account for increased spacing
    y_bottom = y_pos + spacing * 0.4 # Adjusted bottom limit
    ax.set_ylim(y_bottom, y_top)

    ax.set_yticks(yticks)
    # Channel labels: consistent font size and weight for readability
    ax.set_yticklabels(yticklabels, fontsize=8, fontweight='bold')
    ...
    # Panel title: consistent font size (10-12pt) and weight
...
def draw_flowchart(ax):
    ...
    def add_box(x, y, w, h, text, facecolor='#E8F4FD', edgecolor='#2C3E50',
                fontsize=8.5, text_color='#1A2B3C', linewidth=2.0, alpha=0.9): # Base fontsize 8.5 for 7-9pt range
        ...
        line_spacing = min(fontsize * 0.3, h / (n_lines + 0.5)) # Slightly more aggressive line spacing
    ...
    # Adjusted y for better vertical centering of title, h for content, and fontsize
    add_box(5, 9.2, 8.5, 1.3,
            "ChannelPD-Net\nPer-channel CNN+Attention (\u00d718)\n18 PD Probabilities + 18 Frequency Estimates",
            facecolor='#D6EAF8', edgecolor='#2471A3', fontsize=8.5) # Base fontsize for text
    ...
    add_box(2.0, 6.0, 3.2, 2.5,
            ...
            facecolor='#D5F5E3', edgecolor='#1E8449', fontsize=8.5)
    ...
    add_box(5.0, 5.6, 3.2, 3.3,
            ...
            facecolor='#FADBD8', edgecolor='#C0392B', fontsize=8.5)
    ...
    add_box(8.0, 6.0, 3.2, 2.5,
            ...
            facecolor='#FDEBD0', edgecolor='#E67E22', fontsize=8.5)
    ...
    add_arrow(2.0, 4.7, 2.0, 3.75) # y2 = 3.3 (box center) + 0.9/2 (half height) = 3.75
    add_arrow(5.0, 3.9, 5.0, 3.75)
    add_arrow(8.0, 4.7, 8.0, 3.75)

    # ── Output boxes (bottom) ──
    # Raised output boxes for better visual integration with branches
    add_box(2.0, 3.3, 2.8, 0.9, # Raised y from 2.8 to 3.3
            "Laterality",
            facecolor='#D5F5E3', edgecolor='#1E8449', fontsize=9) # Explicit fontsize 9 for single-line output

    add_box(5.0, 3.3, 2.8, 0.9, # Raised y from 2.8 to 3.3
            "Timing + Frequency",
            facecolor='#FADBD8', edgecolor='#C0392B', fontsize=9)

    add_box(8.0, 3.3, 2.8, 0.9, # Raised y from 2.8 to 3.3
            "Spatial Localization",
            facecolor='#FDEBD0', edgecolor='#E67E22', fontsize=9)

    # Title: Vertically centered more closely above the "ChannelPD-Net" box
...
def main():
    ...
    ax_verbal.text(0.5, 0.5, wrapped,
                   ...
                   bbox=dict(boxstyle='round,pad=0.3', facecolor='#f8f8f0',
                             edgecolor='#ccc', alpha=0.9)) # Subtle text box
