...
import matplotlib.patheffects as pe # Added for electrode label outlines
...
def generate_topoplot_on_ax(ax, mean_topo, ch_names_orig, title=''):
    ...
    image, _ = mne.viz.plot_topomap(mean_topo, info, axes=ax, show=False,
                                     ...
                                     vlim=(0, vmax),
                                     head_linewidth=1.5) # Improvement 6: Increased head outline linewidth
    ...
    for i, (orig_name, xy) in enumerate(zip(ch_names_orig, pos)):
        ...
        # Improvement 1: Increase font size and add subtle outline for electrode labels
        ax.text(xy[0], xy[1], orig_name, fontsize=9, ha='center', va='center', # Fontsize increased to 9pt
                fontweight='bold', color=text_color, zorder=10,
                path_effects=[pe.Stroke(linewidth=1.5, foreground='white'), pe.Normal()], # Added white outline
                bbox=dict(boxstyle="round,pad=0.05", fc=(1,1,1,0.4), ec='none')) # Kept subtle transparent background
...
def plot_eeg_traces(ax, eeg_data, title, narrowband=None,
                    highlight_side=None, spacing=220.0): # Improvement 3 & 7: Increased spacing from 180.0 to 220.0
    ...
    for idx in DISPLAY_ORDER:
        if idx == -1:
            ...
            continue
        trace = eeg_data[idx]
        # Improvement 3: Increased EEG trace linewidth
        ax.plot(t, trace + y_pos, color='black', linewidth=1.0, clip_on=True)

        # Green narrowband overlay
        if narrowband is not None:
            nb_trace = narrowband[idx]
            # Improvement 3: Increased narrowband overlay linewidth
            ax.plot(t, nb_trace + y_pos, color='#27AE60', linewidth=1.0,
    ...
    # Improvement 3: Adjusted ylim for increased vertical amplitude
    ax.set_ylim(y_bottom, y_top)

    ax.set_yticks(yticks)
    # Improvement 5: Ensured consistent font size for EEG Channel Labels
    ...
    if highlight_side in ('left', 'right'):
        ...
        if side_y_vals:
            ...
            # Improvement 2: Made hemisphere shading more transparent
            ax.axhspan(y_lo, y_hi, color='lightblue', alpha=0.08, zorder=0)

    # Improvement 5: Ensured consistent font size for panel title
...
def draw_flowchart(ax):
    ...
    def add_box(x, y, w, h, text, facecolor='#E8F4FD', edgecolor='#2C3E50',
                ...
                title_fontsize_boost=1.0, base_line_spacing_factor=0.03):
        ...
        for i, line in enumerate(lines):
            ...
            if i == 0:
                fw = 'bold'
                # Improvement 5: Adjusted title_fontsize_boost for consistent title sizes
                fs = fontsize + title_fontsize_boost
    ...
    # Improvement 5: Adjusted title_fontsize_boost for consistent title sizes
    add_box(2.0, 2.5, 3.0, 2.2,
            "Laterality Detection\nL vs R narrowband amplitude\nOutput: Left/Right\nAUC = 0.837",
            facecolor='#D5F5E3', edgecolor='#1E8449', fontsize=8.5, title_fontsize_boost=2.0)

    # -- Branch 2 (center, purple): Spatial Extent --
    # Improvement 5: Adjusted title_fontsize_boost for consistent title sizes
    add_box(5.0, 2.5, 3.3, 2.5,
            "Spatial Extent\nPLV \u00d7 Amplitude\nPer-channel phase coherence\nwith dominant hemisphere\n\u00d7 narrowband amplitude\nThreshold \u2192 count/18",
            facecolor='#E8DAEF', edgecolor='#7D3C98', fontsize=8.0, title_fontsize_boost=2.0)

    # -- Branch 3 (right, orange): Topographic Localization --
    # Improvement 5: Adjusted title_fontsize_boost for consistent title sizes
    add_box(8.0, 2.5, 3.0, 2.2,
            "Topographic Localization\nPer-channel Hilbert envelope\nLaplacian transform\n\u2192 Topoplot + Verbal Description",
            facecolor='#FDEBD0', edgecolor='#E67E22', fontsize=8.5, title_fontsize_boost=2.0)
    ...
    # -- Output boxes (bottom) --
    # Improvement 5: Ensured consistent font sizes for output boxes
    add_box(2.0, 0.55, 2.8, 0.9,
            "Laterality",
            facecolor='#D5F5E3', edgecolor='#1E8449', fontsize=10, title_fontsize_boost=0)

    add_box(5.0, 0.55, 2.8, 0.9,
            "Spatial Extent + Frequency",
            facecolor='#E8DAEF', edgecolor='#7D3C98', fontsize=9.5, title_fontsize_boost=0)

    add_box(8.0, 0.55, 2.8, 0.9,
            "Spatial Localization",
            facecolor='#FDEBD0', edgecolor='#E67E22', fontsize=10, title_fontsize_boost=0)
...
def main():
    ...
    # Improvement 4: Improve Topoplot Colorbar Presentation
    cbar_ax = inset_axes(ax_topo_inset, width="12%", height="90%", loc='center right', # Made colorbar wider
    ...
    # Improvement 5: Ensured consistent font size for colorbar label and ticks
    cbar_ax.yaxis.label.set_fontsize(9) # Increased colorbar label font size
    cbar_ax.tick_params(labelsize=8) # Increased colorbar tick label font size

    # Verbal description below topoplot inset
    wrapped = verbal
    # Improvement 1: Adjusted line wrapping for increased font size
    if len(verbal) > 35: # Shorter line length threshold
        words = verbal.split()
        lines = []
        current = []
        for w in words:
            current.append(w)
            if len(' '.join(current)) > 30: # Shorter line length threshold
    ...
    # Improvement 1: Increased font size and removed grey background box from verbal description
    fig.text(inset_left + topo_size / 2, inset_bottom - 0.025, wrapped, # Adjusted y position for more space
             ha='center', va='top', fontsize=9, fontstyle='italic', # Increased font size to 9pt
             fontfamily='sans-serif', color='#333') # Removed bbox
