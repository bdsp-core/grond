...
# --- Font Sizes (Critique Point 8: Review Overall Font Sizes for Print Readability) ---
...
CHANNEL_LABEL_FONTSIZE = 9 # Changed from 10 to 9pt (Critique Point 8)
TIME_AXIS_LABEL_FONTSIZE = 9 # Changed from 10 to 9pt (Critique Point 8)
TIME_TICK_LABEL_FONTSIZE = 9 # Changed from 10 to 9pt (Critique Point 8)
VERBAL_DESCRIPTION_FONTSIZE = 9 # Changed from 10 to 9pt (Critique Point 8)
TOPOPLOT_ELECTRODE_LABEL_FONTSIZE = 9 # New constant for Critique Point 2 & 8
COLORBAR_TICK_LABEL_FONTSIZE = 9 # Kept at 9pt (Critique Point 8)
COLORBAR_LABEL_FONTSIZE = 9 # Changed from 10 to 9pt (Critique Point 8)
DIFFICULTY_BADGE_FONTSIZE = 9 # Changed from 10 to 9pt (Critique Point 8)

FIGURE_TITLE_FONTSIZE = 12 # Kept at 12pt, within 10-12pt range (Critique Point 8)
matplotlib.rcParams['figure.titlesize'] = FIGURE_TITLE_FONTSIZE

# --- Spacing Parameters (Critique Point 3: Optimize Horizontal Spacing and Layout) ---
ROW_VSPACE = 0.25 # Kept at 0.25 for vertical spacing between rows
EEG_TO_TOPO_HSPACE = 0.03 # Reduced from 0.06 (Critique Point 3)
TOPO_TO_CBAR_WSPACE = 0.05 # Kept at 0.05
TOPO_INFO_VSPACE = 0.01 # Reduced from 0.02 (Critique Point 3)

# --- Layout Ratios ---
EEG_WIDTH_RATIO = 75
RIGHT_COL_WIDTH_RATIO = 25
TOPO_HEIGHT_RATIO = 6 # Increased from 5 (Critique Point 3)
INFO_HEIGHT_RATIO = 2

# --- Figure Title Placement ---
FIGURE_TITLE_Y_POS = 0.96 # Kept at 0.96
...
# --- Topoplot Electrode Label Styling (Critique Point 2) ---
...
def draw_eeg_panel(ax, case, is_pd):
    ...
    # Hemisphere shading (Critique Point 6: Enhance Visibility)
    lat = case.get('pred_lat', '') or case.get('gt_lat', '')
    subtype = case.get('subtype', '').lower()
    shade_color = (100/255, 160/255, 255/255, 0.15)  # Increased alpha from 0.07
    ...
    for i, entry in enumerate(DISPLAY_ORDER):
        ...
        # Channel label (Critique Point 8: Adjusted font size)
        ax.text(-0.1, yp, entry['name'], fontsize=CHANNEL_LABEL_FONTSIZE, va='center', ha='right',
                color='black', clip_on=False)

    # Amplitude scale bar (double-headed arrow, 100 µV) (Critique Point 5: Relocate)
    from matplotlib.patches import FancyArrowPatch
    scale_uv = 100.0
    scale_height = scale_uv * z_scale  # = 1.2 plot units
    scale_x = 10.2 # Moved from 9.3 to outside the traces
    scale_y_bot = y_min_plot + 0.2 # Moved from y_min_plot + 0.5 to be closer to bottom
    ...
    ax.text(scale_x + 0.15, scale_y_mid, f'{int(scale_uv)} µV',
            fontsize=7, va='center', ha='left', color='black', clip_on=False) # Added clip_on=False

    # Difficulty badge (Critique Point 7: Ensure Consistent Agreement Label Alignment)
    ...
    ax.text(0.99, 0.99, diff_text, transform=ax.transAxes, # Adjusted y from 0.98 to 0.99
            fontsize=DIFFICULTY_BADGE_FONTSIZE, fontweight='bold', # Critique Point 8: Adjusted font size
    ...
    # Time axis labels (Critique Point 8: Adjusted font size)
...
def draw_topoplot(ax, ax_cbar, case, is_pd):
    ...
    # Determine vmin, vmax for colorbar (Critique Point 1)
    vmin, vmax = None, None
    if is_pd:
    ...
    if topo_b64:
        ...
        image, _ = mne.viz.plot_topomap(
            ...
            show_names=False, # Disable MNE's names to draw our own (Critique Point 2)
        )
        ax.axis('off') # Ensure axis is off even if MNE doesn't fully hide it

    # --- Add Electrode Labels (Critique Point 2: Improve Readability) ---
    ...
    # --- Add Colorbar (Critique Point 1: Add Clear Colorbar) ---
    ...
    cbar.ax.tick_params(labelsize=COLORBAR_TICK_LABEL_FONTSIZE) # Critique Point 8: Adjusted font size
    cbar.set_label('Score', fontsize=COLORBAR_LABEL_FONTSIZE) # Critique Point 8: Adjusted font size
...
def draw_info_panel(ax, case):
    ...
    if verbal:
        ...
        ax.text(0.5, 1.0, wrapped, fontsize=VERBAL_DESCRIPTION_FONTSIZE, ha='center', va='top',
                linespacing=1.6, transform=ax.transAxes) # Critique Point 4: Increased linespacing from 1.4 to 1.6
...
def render_subtype(subtype, cases, is_pd):
    ...
    fig_width = 18
    fig_height = 6.5 * n_cases # Adjusted height per case for better spacing
    ...
    outer_gs = gridspec.GridSpec(n_cases, 2, figure=fig,
                                 ...
                                 hspace=ROW_VSPACE,
                                 wspace=EEG_TO_TOPO_HSPACE, # Critique Point 3: Optimized horizontal spacing
    ...
    for row, case in enumerate(cases):
        ...
        inner_gs_right_col = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=outer_gs[row, 1],
                                                               ...
                                                               hspace=TOPO_INFO_VSPACE) # Critique Point 3: Optimized vertical spacing
