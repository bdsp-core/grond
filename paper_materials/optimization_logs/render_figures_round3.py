...
def draw_eeg_panel(ax, case, is_pd):
    ...
    # Add EEG Amplitude Scale (Critique Point 1: Add EEG Amplitude Scale)
    scale_uv = 50.0 # Example scale: 50 µV
    scale_height_plot_units = scale_uv * z_scale
    
    # Position: 5% from left, 90% from bottom of the axes (in axes coordinates)
    x_ax_coord = 0.05
    y_ax_coord_top = 0.9
    
    # Convert axes coordinates to data coordinates for plotting
    inv_transform = ax.transAxes.inverted()
    x_data_top, y_data_top = inv_transform.transform((x_ax_coord, y_ax_coord_top))
    y_data_bottom = y_data_top - scale_height_plot_units
    
    # Draw the vertical line for the scale bar
    ax.plot([x_data_top, x_data_top], [y_data_top, y_data_bottom], 
            color='black', linewidth=1.5, solid_capstyle='butt', zorder=3)
    
    # Add the text label "50 µV"
    ax.text(x_data_top, y_data_top, f'{int(scale_uv)} µV', 
            fontsize=CHANNEL_LABEL_FONTSIZE, va='bottom', ha='left', 
            color='black', clip_on=False,
            bbox=dict(facecolor='white', edgecolor='none', pad=1)) # Small white background for clarity

    # EEG Data Summary Text (Critique Point 2: Increased font size)
...
def draw_info_panel(ax, case):
    ...
    full_text = '\n'.join(text_parts)
    # Verbal descriptions (Critique Point 2: Increased font size)
