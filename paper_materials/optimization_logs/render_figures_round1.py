...
# --- Global Matplotlib Styling (Point 4: Standardize Font Family and Weight) ---
matplotlib.rcParams['font.family'] = 'sans-serif'
matplotlib.rcParams['font.sans-serif'] = ['Arial', 'Helvetica', 'DejaVu Sans', 'Liberation Sans', 'Bitstream Vera Sans']
# Set base font sizes, then override specifically where needed based on critique
matplotlib.rcParams['font.size'] = 8 # Base font size for general text (e.g., tick labels, general text)
matplotlib.rcParams['axes.labelsize'] = 8 # For x/y axis labels
matplotlib.rcParams['xtick.labelsize'] = 8 # For x-axis tick labels
matplotlib.rcParams['ytick.labelsize'] = 8 # For y-axis tick labels (though mostly hidden here)
matplotlib.rcParams['figure.titlesize'] = 12 # For main figure titles (Point 2: 10-12pt)
...
def draw_eeg_panel(ax, case, is_pd):
    ...
    for i, entry in enumerate(DISPLAY_ORDER):
        ...
        # Channel label (Point 2: Increased font size; Point 3: Closer proximity)
        ax.text(-0.1, yp, entry['name'], fontsize=8, va='center', ha='right',
                color='black', clip_on=False)

    # EEG Data Summary Text (Point 5, 7: Moved from title, left-aligned, increased font size)
    ...
    ax.text(0.01, 0.93, data_summary_text, transform=ax.transAxes,
            fontsize=9, va='top', ha='left', color='#333') # Font size 7-9pt

    # Difficulty badge (Point 5: Refined design, separated from data summary, increased font size)
    ...
    ax.text(0.99, 0.98, diff_text, transform=ax.transAxes,
            fontsize=9, fontweight='bold', color=dc, va='top', ha='right') # Font size 7-9pt, no bbox

    # Time axis labels (Point 2: Increased font size)
    ax.set_xlabel('Time (s)', fontsize=8) # Font size 7-9pt
    ax.set_xticks(range(11))
    ax.set_xticklabels([str(i) for i in range(11)], fontsize=8) # Font size 7-9pt
    ax.set_yticks([])
...
def draw_topoplot(ax, ax_cbar, case, is_pd): # Modified signature to include ax_cbar and is_pd
    ...
    # Determine vlim based on subtype for consistent scaling (Point 1)
    vmin, vmax = None, None
    if is_pd: # LPD or GPD
    ...
    image, _ = mne.viz.plot_topomap( # Capture image object for colorbar
        ...
        # Topoplot Electrode Labels (Point 2) are affected by global rcParams['font.size']
    )
    
    # Add colorbar (Point 1: Clear Colorbar with Scale)
    cbar = plt.colorbar(image, cax=ax_cbar, orientation='vertical', label='Score')
    cbar.ax.tick_params(labelsize=8) # Colorbar tick labels (Point 2: 7-9pt)
    cbar.set_label('Score', fontsize=9) # Colorbar label (Point 2: 7-9pt)
...
def draw_info_panel(ax, case):
    ...
    full_text = '\n'.join(text_parts)
    # Verbal descriptions (Point 2: Increased font size)
    ax.text(0.5, 1.0, full_text, fontsize=7, ha='center', va='top',
...
def render_subtype(subtype, cases, is_pd):
    ...
    fig_width = 18
    fig_height = 6 * n_cases # Reduced height for compactness (Point 3)
    fig = plt.figure(figsize=(fig_width, fig_height), facecolor='white')
    
    # Main Figure Titles (Point 2: Increased font size; Point 3: Adjusted spacing)
    fig.suptitle(f'{subtype.upper()} Characterization Examples',
                 fontsize=matplotlib.rcParams['figure.titlesize'], fontweight='bold', y=0.98)

    # 3 rows, each row: [EEG (75%) | topoplot + info (25%)]
    # Optimize Layout and Spacing (Point 3)
    outer_gs = gridspec.GridSpec(n_cases, 2, figure=fig,
                                 width_ratios=[75, 25],
                                 hspace=0.15, wspace=0.03, # Reduced vertical and horizontal spacing
    ...
    for row, case in enumerate(cases):
        ...
        # This inner_gs now manages the overall right column layout
        inner_gs_right_col = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=outer_gs[row, 1],
                                                               height_ratios=[3, 2], hspace=0.02) # Reduced hspace

        # Topoplot and Colorbar (Point 1: Add a Clear Colorbar with Scale)
        # Create a new gridspec for the topoplot and its colorbar
        topoplot_cbar_gs = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=inner_gs_right_col[0],
                                                             width_ratios=[10, 1], wspace=0.05) # Topo (10) vs Cbar (1)

        ax_topo = fig.add_subplot(topoplot_cbar_gs[0])
        ax_cbar = fig.add_subplot(topoplot_cbar_gs[1]) # Axis for the colorbar

        # Pass ax_cbar and is_pd to draw_topoplot
