...
\textbf{Main results.} The system matched or exceeded expert inter-rater agreement on every PD task and on GRDA frequency, and approached expert agreement on LRDA. In a prospective independent-rater evaluation in which three electroencephalographers not involved in algorithm development scored 200 stratified segments per pattern subtype, with analysis restricted to segments where at least 2 of 3 raters accepted the segment as a valid instance, the algorithm \emph{exceeded} expert--expert intraclass correlation coefficients (ICCs) on LPD frequency (mean expert--algorithm ICC 0.916 vs.\ mean expert--expert 0.866; paired segment-bootstrap $\Delta=+0.051$, 95\% confidence interval (CI) $[+0.026, +0.080]$, $p<0.001$) and GPD frequency (0.975 vs.\ 0.966; $\Delta=+0.009$, $[+0.003,+0.017]$, $p<0.001$). On GRDA frequency the algorithm \emph{matched} expert--expert ICC (0.942 vs.\ 0.944; $p=0.55$). On LRDA frequency the retuned NB-Hilbert estimator reached an expert--algorithm ICC of 0.815 versus expert--expert 0.894 ($\Delta=-0.079$, $[-0.184,-0.000]$, $p=0.048$ -- modestly but significantly below the expert--expert level). On LRDA laterality, our hybrid V14 rule -- which combines an amplitude-based default with a rhythmicity-based unanimous override (see Methods) -- reached $\kappa=0.948$ vs.\ expert--expert $\kappa=0.983$ ($\Delta=-0.035$, $[-0.088,+0.006]$, $p=0.116$ -- statistically indistinguishable from the expert--expert level after MW's two-pass disagreement review tightened the algorithm's match to the human consensus on this task). PD discharge timing achieved an F1 of 0.889 at single-sample precision (1.0~ms median timing error at 200~Hz). PD hemisphere lateralization reached an area under the receiver operating characteristic curve (AUC) of 0.989 ($n = 1{,}274$), and LPD versus GPD classification reached an AUC of 0.911 ($n = 7{,}037$). RDA lateralization reached an AUC of 0.837 ($n = 4{,}253$). Spatial localization reached 97.3\% of expert inter-rater agreement for PDs (Jaccard 0.731 versus expert--expert 0.751) and matched expert--expert ICC for RDA (0.371 versus 0.373).
...
\textbf{Annotation strategy and rater coverage.} The labeled corpus is dominated by a single rater (MW), who produced per-segment labels using the new interactive narrowband-overlay tools developed for the present work; these tools display the candidate frequency as a moving narrowband filter visually matched against the EEG envelope, making frequency estimation substantially easier and more accurate than the conventional approach used in prior work~\cite{tautan2025}. A subset of segments was reviewed in multiple passes, including review of cases where the algorithm disagreed with the prior MW label, to identify and correct systematic label noise. To verify that the resulting algorithm performs at expert level on raters and labels independent of MW, we report two complementary inter-rater evaluations:

\textbf{(i) Canonical 3-rater independent-expert evaluation (MW, SZ, TZ).} We commissioned an independent task in which two co-author electroencephalographers (SZ and TZ) -- neither of whom contributed to algorithm development -- scored lateralization, frequency, and discharge timing on 200 stratified segments per pattern subtype (LPD, GPD, LRDA, GRDA; figure~\ref{fig:indep_expert_freq}). Each rater independently scored every segment and recorded a per-segment accept/reject decision (rejected segments were judged by the rater not to be valid instances of the labeled subtype). The canonical inter-rater reliability (IRR) analysis includes only segments where at least 2 of the 3 independent raters (MW + SZ + TZ) accepted the segment as a valid instance, removing dataset-labeling noise from the IRR computation; an individual rater's labels are used only on segments where that rater also accepted the segment. Under this canonical inclusion rule, segment counts per task are LPD~195/200, GPD~198/200, LRDA~155/200, and GRDA~132/200; rejection rates differed by rater (MW 0--13\%, SZ 5--44\%, TZ 7--35\%) with the highest rates on LRDA and GRDA. Inter-rater reliability is reported in figure~\ref{fig:irr_main} (paired EE vs.\ EA bars with bootstrap 95\% CIs and significance tests) and in detail in the supplementary analysis document (\texttt{paper\_materials/independent\_expert\_tasks/analysis\_v1.md}).

\textbf{(ii) Independent prior-cohort evaluation (PH, LB, SZ; Tautan et al.~\cite{tautan2025} dataset).} As a fully MW-independent check on the same algorithm, we additionally evaluated against the three expert raters (PH, LB, SZ) who labeled the Tautan et al.\ dataset. These labels were produced without the interactive narrowband-overlay tools described above and are therefore noisier than the canonical cohort by construction; the cohort's expert--expert ICCs (LPD 0.40, GPD 0.61, LRDA 0.57, GRDA 0.79) reflect this. The algorithm's agreement with these three raters is reported in table~\ref{tab:phlbsz_cohort}.
...
\textbf{PD frequency.} On quality-filtered segments, the PD-Profiler (IPI-derived frequency from HemiCET-UNet + DP) achieved Spearman correlations of 0.772 for LPD ($n = 1{,}103$, MAE $= 0.170$~Hz) and 0.819 for GPD ($n = 1{,}099$, MAE $= 0.146$~Hz) (table~\ref{tab:frequency}, figure~\ref{fig:freq_scatter}). These results represented substantial improvements over the signal-processing baseline of Tautan et al.~\cite{tautan2025}, which achieved correlations of 0.278 (LPD, MAE $= 0.505$~Hz) and 0.509 (GPD, MAE $= 0.339$~Hz) on the same labeled cohort -- approximately a 2.8-fold improvement on LPD and 1.6-fold on GPD.
...
\textbf{RDA frequency.} The NB-Hilbert iterative narrowband method (V12: pass-1 0.5--4.5~Hz, pass-2 narrowband half-width 0.5~Hz, top-3 channels, frequency cap 4.5~Hz) achieved Spearman correlations of 0.719 for LRDA ($n = 721$, MAE $= 0.225$~Hz) and 0.795 for GRDA ($n = 1{,}453$, MAE $= 0.188$~Hz) on quality-filtered segments (table~\ref{tab:frequency}, figure~\ref{fig:freq_scatter}). Tautan et al.\ achieved correlations of 0.075 (LRDA, MAE $= 0.623$~Hz) and 0.132 (GRDA, MAE $= 0.578$~Hz) on the same cohort -- a 9.6-fold improvement on LRDA and 6.0-fold on GRDA.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/fig4_frequency_scatter.png}
\caption{Algorithm frequency estimates correlate substantially more strongly with expert labels than the prior signal-processing state of the art. Scatter plots comparing algorithm-predicted frequency ($y$-axis) against expert-reviewed frequency ($x$-axis) for each subtype. Top row: present system (PD: CNN+ACF frequency prior refined by IPI from detected discharges; RDA: NB-Hilbert iterative narrowband Hilbert refinement, V12 hyperparameters). Bottom row: T\u{a}u\c{t}an et al.\ signal-processing baseline. Columns from left to right: LPD ($n = 1{,}103$; $\rho = 0.772$), GPD ($n = 1{,}099$; $\rho = 0.819$), LRDA ($n = 721$; $\rho = 0.719$), GRDA ($n = 1{,}453$; $\rho = 0.795$). Spearman correlation ($\rho$) and mean absolute error (MAE, in Hz) are shown for each panel. Expert frequency is the mean across available raters in labels.csv. The Tautan baseline was rerun on the same labeled cohort (per-row $n$ in the figure title differs slightly between methods because a small minority of segments produced an estimate from one method but not the other). Dashed diagonal line indicates perfect agreement.}
...
\caption{Algorithm--expert agreement matches or exceeds the expert--expert ceiling on the canonical independent-expert cohort. Mean inter-rater reliability for each (task, metric) computed under the canonical majority-accept consensus rule on the 200-segment-per-subtype independent-expert annotation cohort. \textbf{(A)} Frequency ICC(3,1). \textbf{(B)} Frequency Spearman $\rho$. \textbf{(C)} Laterality Cohen $\kappa$ (LPD and LRDA only; GPD and GRDA are bilateral by definition). Blue bars: mean of the three expert--expert pairs (MW--SZ, MW--TZ, SZ--TZ). Orange bars: mean of the three expert--algorithm pairs. Error bars: 95\% CI on the mean from a paired segment-level bootstrap (2{,}000 resamples). Open circles: individual pair-wise point estimates. Significance brackets compare the EA mean against the EE mean using the same paired bootstrap (sign-corrected so $+$ favors the algorithm; *** $p<0.001$, ** $p<0.01$, * $p<0.05$; n.s. = not significant at $\alpha=0.05$). The algorithm uses V12 frequency estimation (NB-Hilbert with retuned hyperparameters: pass-1 0.5--4.5 Hz, pass-2 narrowband half-width 0.5 Hz, top-3 dominant-hemisphere channels averaged, frequency cap 4.5 Hz) and the V14 amplitude/rhythmicity hybrid laterality rule (default to the V12 amplitude call; override only when four amplitude-normalized rhythmicity measures -- per-channel Q-factor, within-hemisphere phase-locking value, narrowband peak-amplitude consistency, and spectral peak prominence -- \emph{unanimously} disagree with the amplitude call).}
\label{fig:irr_main}
\end{figure}

\textbf{Validation against the prior 3-rater cohort (Tautan et al.\ 2025).}
As an additional check independent of the primary annotator (MW), we evaluated the algorithm against the three expert raters (PH, LB, SZ) of the Tautan et al.~\cite{tautan2025} cohort, whose labels were produced without the present interactive narrowband-overlay tools and are therefore noisier than the canonical cohort by construction. Mean agreement across the three pairs (table~\ref{tab:phlbsz_cohort}) shows that the algorithm clearly exceeds expert--expert reliability on RDA frequency: mean expert--algorithm ICC 0.73 versus mean expert--expert ICC 0.57 on LRDA ($n=162$--$210$ per pair) and 0.89 versus 0.79 on GRDA ($n=247$--$282$). On LPD frequency the algorithm is comparable to expert--expert agreement on this older cohort (mean EA ICC 0.51 vs.\ mean EE ICC 0.40), with the modest absolute reliability of the EE ceiling reflecting the noisiness of the labels themselves. On GPD frequency the algorithm tracks SZ tightly (ICC 0.83) but disagrees more substantially with PH and LB (ICCs 0.06 and $-0.07$ respectively); the per-pair MAEs (PH--ALG 0.45 Hz, LB--ALG 0.44 Hz, SZ--ALG 0.22 Hz) are consistent with a scoring-convention difference on this subtype. The Tautan et al.\ signal-processing baseline is uniformly weaker than the present algorithm on this cohort (mean expert--Tautan ICC: LPD 0.13, GPD 0.35, LRDA $-0.29$, GRDA 0.18). On a fully MW-independent labeled set, the present algorithm therefore meets or exceeds expert-vs-expert reliability for RDA frequency tasks and on LPD frequency, with a residual subtype-specific scoring divergence on GPD that warrants future investigation.

\begin{table}[t]
\caption{Algorithm validation on the independent Tautan et al.~\cite{tautan2025} 3-rater cohort (raters PH, LB, SZ). Mean across the three within-role pairs (PH--LB, PH--SZ, LB--SZ for EE; PH--ALG, LB--ALG, SZ--ALG for EA; PH--TAU, LB--TAU, SZ--TAU for ET). The cohort is fully independent of the primary annotator (MW) and was labeled without the interactive narrowband-overlay tools introduced here.}
...
\hline
LPD freq  & 0.40 & 0.61 & 0.31 & \textbf{0.51} & 0.58 & 0.28 &  0.13 & 0.22 & 0.50 \\
GPD freq  & 0.61 & 0.47 & 0.20 & 0.27 & 0.34 & 0.37 &  0.35 & 0.35 & 0.30 \\
LRDA freq & 0.57 & 0.66 & 0.24 & \textbf{0.73} & \textbf{0.74} & \textbf{0.19} & $-0.29$ & 0.17 & 0.77 \\
GRDA freq & 0.79 & 0.75 & 0.19 & \textbf{0.89} & \textbf{0.86} & \textbf{0.16} &  0.18 & $-0.09$ & 0.57 \\
\hline
\end{tabular}
\end{table}

\textbf{Re-evaluation against independent raters.} The independent-expert task described above included 200 segments per subtype (LPD, GPD, LRDA, GRDA). Under the canonical majority-accept consensus rule, the algorithm reached PD-frequency expert--algorithm ICCs of 0.869 (MW), 0.976 (SZ), and 0.903 (TZ) for LPD ($n=168$--$187$ per pair) and 0.964 (MW), 0.987 (SZ), and 0.976 (TZ) for GPD ($n=187$--$197$ per pair). All six PD-frequency expert--algorithm ICCs are at or above their corresponding expert--expert ICCs, indicating that the PD-frequency apparent gap noted earlier (algorithm--MW ICC 0.572 vs.\ expert--expert ICC 0.662 in the original 68-segment three-rater RDA-only analysis) does not generalize to the broader independent expert evaluation. The earlier 0.572 figure was specific to a three-rater PD-frequency micro-cohort dominated by MW labels; the present analysis with two fully independent raters shows the algorithm matching or exceeding expert--expert agreement on PD frequency. On RDA, the algorithm reached GRDA-frequency expert--algorithm ICCs of 0.921 (MW), 0.989 (SZ), and 0.917 (TZ) ($n=122$--$123$ per pair), at the expert--expert ceiling. For LRDA the originally chosen NB-Hilbert hyperparameters (pass-1 bandpass 0.5--3.5~Hz; pass-2 narrowband half-width 0.4~Hz) had been selected during the early algorithm contest using a small three-rater cohort. With the expanded 156-segment majority-accept consensus dataset now available, we re-tuned these hyperparameters via a 192-point grid search (pass-1 bandpass upper $\in\{3.5,4.0,4.5,5.0\}$~Hz; pass-2 half-width $\in\{0.3,0.4,0.5,0.6\}$~Hz; top-$K$ channels $\in\{2,3,4,5\}$; frequency search cap $\in\{3.5,4.0,4.5\}$~Hz). The retuned configuration (pass-1 0.5--4.5~Hz; pass-2 half-width 0.5~Hz; top-3 channels; cap 4.5~Hz) reached LRDA expert--algorithm ICCs of 0.769 (MW), 0.865 (SZ), and 0.812 (TZ) for a mean expert--algorithm ICC of 0.815 against expert--expert 0.894; this remains modestly but significantly below the expert--expert ceiling ($\Delta=-0.079$, 95\% CI $[-0.184,-0.000]$, $p=0.048$). The expert ceiling itself shifted during this work: MW conducted a two-pass review of every LRDA segment where the V14 frequency disagreed with any rater. The first pass covered all 425 segments with $>0.25$~Hz max-rater disagreement; the second pass focused on the 132 segments where MW specifically disagreed by $>0.5$~Hz, of which MW retracted 54 as not-LRDA and adjusted the frequency on a further 22. The resulting MW--TZ frequency ICC sharpened from 0.921 to 0.955; the LRDA-laterality MW--TZ $\kappa$ moved from 1.000 to 0.985 (the previously perfect MW--TZ laterality agreement now reflects MW's revised calls on a few previously-mis-lateralized segments). For LRDA laterality, the original W05 rule lateralizes by amplitude (which hemisphere has more narrowband envelope amplitude at the estimated rhythm frequency); this is fallible by construction whenever artifact, slow drift, or asymmetric volume conduction inflates one hemisphere's amplitude even when the rhythm itself is on the other side. We therefore developed V14, a hybrid that defaults to the amplitude rule but overrides it whenever four amplitude-normalized rhythmicity measures (per-channel Q-factor, within-hemisphere phase-locking value, narrowband peak-amplitude consistency, and spectral peak prominence) \emph{unanimously} disagree with the amplitude call. On the 150-segment consensus set V14 flips 4 of the V12 calls; this gives LRDA-laterality expert--algorithm $\kappa=0.948$ (MW: 0.920; SZ: 0.982; TZ: 0.941) vs.\ expert--expert $\kappa=0.983$. To formally test whether the algorithm--expert agreement differs from expert--expert agreement on each task, we performed a paired segment-level bootstrap (2000 resamples) of $\Delta = \mathrm{mean}_\mathrm{EA} - \mathrm{mean}_\mathrm{EE}$ (sign-corrected so positive favors the algorithm). The algorithm exceeded the expert--expert ceiling at significance on LPD ICC ($\Delta=+0.051$, 95\% CI $[+0.026,+0.080]$, $p<0.001$), GPD ICC ($\Delta=+0.009$, $[+0.003,+0.017]$, $p<0.001$), LPD Spearman ($\Delta=+0.064$, $p<0.001$), and GPD Spearman ($\Delta=+0.010$, $p=0.011$); was statistically indistinguishable from the expert ceiling on GRDA ICC ($p=0.55$), GRDA Spearman ($p=0.74$), LPD MAE ($p=0.93$), LPD laterality $\kappa$ ($p=0.29$), and -- after the V14 hybrid plus MW's review pass -- LRDA laterality $\kappa$ ($\Delta=-0.035$, $[-0.088,+0.006]$, $p=0.116$); and was significantly below the expert ceiling on LRDA frequency ($\Delta=-0.079$, $p=0.048$), LRDA frequency Spearman, and RDA MAE. A summary visualization with paired EE/EA bars, 95\% CIs on the means, and significance markers is shown in Figure~\ref{fig:irr_main}.
...
\hline
LPD & 1,103 & 0.772 & 0.170 & 0.278 & 0.505 \\
GPD & 1,099 & 0.819 & 0.146 & 0.509 & 0.339 \\
LRDA & 721 & 0.719 & 0.225 & 0.075 & 0.623 \\
GRDA & 1,453 & 0.795 & 0.188 & 0.132 & 0.578 \\
\hline
\end{tabular}
\label{tab:frequency}
\end{table}
