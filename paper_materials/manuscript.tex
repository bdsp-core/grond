...
\textbf{Main results.} The system matched or exceeded expert inter-rater agreement on every PD task and on GRDA frequency, and approached expert agreement on LRDA. In a prospective independent-rater evaluation (preliminary three-rater results; a fourth rater is currently completing the same task) in which electroencephalographers not involved in algorithm development scored 200 stratified segments per pattern subtype, with analysis restricted to segments where at least 2 of 3 raters accepted the segment as a valid instance, the algorithm \emph{exceeded} expert--expert ICCs on LPD frequency (mean expert--algorithm ICC 0.916 vs.\ mean expert--expert 0.866; paired segment-bootstrap $\Delta=+0.051$, 95\% CI $[+0.026, +0.080]$, $p<0.001$) and GPD frequency (0.975 vs.\ 0.966; $\Delta=+0.009$, $[+0.003,+0.017]$, $p<0.001$). On GRDA frequency the algorithm \emph{matched} expert--expert ICC (0.942 vs.\ 0.944; $p=0.55$). On LRDA frequency the retuned NB-Hilbert estimator reached an expert--algorithm ICC of 0.814 versus expert--expert 0.895 ($\Delta=-0.080$, $[-0.185,-0.002]$, $p=0.040$ -- modestly but significantly below the expert--expert level after MW's recent self-corrections sharpened MW--TZ agreement from 0.921 to 0.950). On LRDA laterality, our hybrid V14 rule -- which combines an amplitude-based default with a rhythmicity-based unanimous override (see Methods) -- closed about 40\% of the residual gap to the expert--expert ceiling, reaching $\kappa=0.953$ vs.\ expert--expert $\kappa=0.994$ ($\Delta=-0.041$, $[-0.091,-0.005]$, $p=0.036$). PD discharge timing achieved an F1 of 0.889 at single-sample precision (1.0~ms median timing error at 200~Hz). PD hemisphere lateralization reached an area under the receiver operating characteristic curve (AUC) of 0.989 ($n = 1{,}274$), and LPD versus GPD classification reached an AUC of 0.911 ($n = 7{,}037$). RDA lateralization reached an AUC of 0.837 ($n = 4{,}253$). Spatial localization reached 97.3\% of expert inter-rater agreement for PDs (Jaccard 0.731 versus expert--expert 0.751) and matched expert--expert ICC for RDA (0.371 versus 0.373).
...
LRDA & 726 & 0.687 & 0.232 & 0.070 & 0.622 \\
