...
\textbf{Main results.} The system matched or exceeded expert inter-rater agreement on every PD task and on GRDA frequency, and approached expert agreement on LRDA. In a prospective independent-rater evaluation in which three electroencephalographers not involved in algorithm development (SZ, TZ, AS) scored 200 stratified segments per pattern subtype alongside the primary annotator (MW), with analysis restricted to segments where the per-segment accept count exceeded the per-segment reject count across the four raters, the algorithm \emph{exceeded} expert--expert intraclass correlation coefficients (ICCs) on LPD frequency (mean expert--algorithm ICC 0.931 vs.\ mean expert--expert 0.897; paired segment-bootstrap $\Delta=+0.034$, 95\% confidence interval (CI) $[+0.016, +0.055]$, $p<0.001$). On GPD frequency the algorithm matched the expert--expert ceiling (mean EA ICC 0.980 vs.\ EE 0.973; $\Delta=+0.007$, $p=0.061$, n.s.). On GRDA frequency the algorithm was statistically indistinguishable from the expert--expert ceiling (0.926 vs.\ 0.946; $\Delta=-0.020$, $p=0.056$, n.s.). On LRDA frequency the retuned NB-Hilbert estimator reached mean EA ICC 0.854 vs.\ EE 0.914 ($\Delta=-0.055$, $p=0.209$, n.s.). On LRDA laterality, our hybrid V14 rule -- which combines an amplitude-based default with a rhythmicity-based unanimous override (see Methods) -- reached $\kappa=0.961$ vs.\ expert--expert $\kappa=0.993$ ($\Delta=-0.032$, $[-0.087,+0.004]$, $p=0.269$, n.s.). The algorithm was therefore statistically indistinguishable from or significantly above the four-rater expert--expert ceiling on every frequency and laterality task evaluated. PD discharge timing achieved an F1 of 0.889 at single-sample precision (1.0~ms median timing error at 200~Hz). PD hemisphere lateralization reached an area under the receiver operating characteristic curve (AUC) of 0.989 ($n = 1{,}274$), and LPD versus GPD classification reached an AUC of 0.911 ($n = 7{,}037$). RDA lateralization reached an AUC of 0.837 ($n = 4{,}253$). Spatial localization reached 97.3\% of expert inter-rater agreement for PDs (Jaccard 0.731 versus expert--expert 0.751) and matched expert--expert ICC for RDA (0.371 versus 0.373).
...
\caption{Algorithm--expert agreement matches or exceeds the expert--expert ceiling on two independent 4-rater cohorts. Each bar is the mean across within-role pairs computed by paired segment-level bootstrap (2{,}000 resamples); error bars are 95\% CI on the mean from the same bootstrap. Open circles are individual pair-wise point estimates. Significance brackets test mean(EA)$-$mean(EE) (sign-corrected so $+$ favors the algorithm; *** $p<0.001$, ** $p<0.01$, * $p<0.05$; n.s.\ = not significant at $\alpha=0.05$). \textbf{Top row: canonical 4-rater independent-expert cohort (MW, SZ, TZ, AS)} on 200 stratified segments per pattern subtype, restricted to segments where the per-segment accept count exceeded the per-segment reject count. \textbf{(A)}~Frequency Spearman $\rho$ (LPD, GPD, GRDA, LRDA); 6 EE pairs $\times$ 4 EA pairs. \textbf{(B)}~Laterality Cohen $\kappa$ (LPD and LRDA only; GPD and GRDA are bilateral by definition). \textbf{Bottom row: prior 4-rater Tautan et al.~\cite{tautan2025} cohort (PH, LB, SZ, MW)}, where PH/LB/SZ produced their labels without the present interactive narrowband-overlay tools. \textbf{(C)}~Frequency Spearman $\rho$. Laterality is not shown for the prior cohort because PH and LB did not annotate laterality. The algorithm uses V12 frequency estimation (NB-Hilbert with retuned hyperparameters: pass-1 0.5--4.5 Hz, pass-2 narrowband half-width 0.5 Hz, top-3 dominant-hemisphere channels averaged, frequency cap 4.5 Hz) and the V14 amplitude/rhythmicity hybrid laterality rule (default to the V12 amplitude call; override only when four amplitude-normalized rhythmicity measures -- per-channel Q-factor, within-hemisphere phase-locking value, narrowband peak-amplitude consistency, and spectral peak prominence -- \emph{unanimously} disagree with the amplitude call). Per-pair ICCs and MAEs are reported in the main text and in table~\ref{tab:phlbsz_cohort}.}
\label{fig:irr_main}
\end{figure}
