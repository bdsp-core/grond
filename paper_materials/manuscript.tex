...
\caption{Independent-expert v1 inter-rater reliability: pairwise expert--expert (red circles) and expert--algorithm (green squares) agreement on each task, computed under the canonical majority-accept consensus rule (a segment enters the analysis only if $\geq 2$ of 3 raters accepted it as a valid instance of the labeled pattern; per-rater labels from any rater who individually rejected a segment are also dropped). Each row shows the three pair-wise estimates; horizontal bars span the bootstrap 95\% CI envelope. The algorithm column uses V12, the NB-Hilbert estimator with hyperparameters retuned against the expanded majority-accept consensus dataset (pass-1 bandpass 0.5--4.5~Hz; pass-2 narrowband half-width 0.5~Hz; top-3 dominant-hemisphere channels averaged; frequency search cap 4.5~Hz). Significance of the algorithm--expert versus expert--expert difference (paired segment-bootstrap, 2000 resamples) is reported in the main text.}
