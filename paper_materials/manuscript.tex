...
\textbf{3-way PD classification (LPD vs.\ GPD vs.\ BIPD).} BIPD detection is a small, exploratory component of the present work: the dataset comprised 4{,}652 LPD, 3{,}331 GPD, and only 10 confirmed BIPD segments (21 confirmed BIPD cases were screened in total; 11 were retained for training and 10 used for held-out evaluation). The 3-way classifier incorporating timing-derived features reached a macro AUC of 0.888 ($n = 7{,}993$; per-class one-vs-rest AUCs LPD 0.871, GPD 0.872, BIPD 0.922); in the binary BIPD-vs-GPD comparison the classifier reached an AUC of 0.936 ($n = 3{,}341$). With only 10 held-out BIPD segments the BIPD-side AUC point estimates are highly uncertain: by the Hanley--McNeil approximation, 95\% confidence intervals are LPD $[0.863, 0.879]$, GPD $[0.863, 0.880]$, BIPD one-vs-rest $[0.807, 1.000]$, BIPD vs.\ GPD $[0.831, 1.000]$, and the overall macro AUC is dominated by the wide BIPD interval. The most discriminative features for BIPD detection were the matched fraction (proportion of cross-hemisphere coincident discharges within 100~ms) and the phase consistency between left and right hemisphere discharge trains, which capture the defining characteristic of temporal independence between hemispheres. We caution that all BIPD numbers reported here are preliminary; the synthetic-data training procedure (\S\ref{sec:bipd}) plausibly transfers to real BIPD because the timing-feature distinctions used by the classifier (independent vs.\ tightly time-locked discharge trains) are well-represented in the synthetic positive class, but a definitive evaluation will require a substantially larger held-out cohort of confirmed real BIPD segments.
...
3-way macro (LPD/GPD/BIPD) & RF on channel probs + timing & 0.888$^\dagger$ & 7,993 \\
BIPD vs.\ GPD & Timing features (GBT) & 0.936 [0.83, 1.00]$^\dagger$ & 3,341 \\
