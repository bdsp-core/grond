...
\textbf{Main results.} The system matched or exceeded expert inter-rater agreement on every PD task and on GRDA frequency, and approached expert agreement on LRDA. In a prospective independent-rater evaluation (preliminary three-rater results; a fourth rater is currently completing the same task) in which electroencephalographers not involved in algorithm development scored 200 stratified segments per pattern subtype, the algorithm reached expert--algorithm ICCs comparable to expert--expert ICCs on LPD frequency (mean expert--algorithm ICC 0.916, mean expert--expert 0.866) and GPD frequency (mean expert--algorithm 0.976, mean expert--expert 0.966), with the analysis restricted to segments where at least 2 of 3 raters accepted the segment as a valid instance. On LRDA the algorithm achieved an expert--algorithm frequency ICC of 0.802 versus an expert--expert ICC of 0.892, and an expert--algorithm laterality $\kappa$ of 0.923 versus an expert--expert $\kappa$ of 0.994 -- a small but real residual gap on LRDA driven by harmonic ambiguity in fast LRDA segments. PD discharge timing achieved an F1 of 0.889 at single-sample precision (1.0~ms median timing error at 200~Hz). PD hemisphere lateralization reached an area under the receiver operating characteristic curve (AUC) of 0.989 ($n = 1{,}274$), and LPD versus GPD classification reached an AUC of 0.911 ($n = 7{,}037$). RDA lateralization reached an AUC of 0.837 ($n = 4{,}253$). Spatial localization reached 97.3\% of expert inter-rater agreement for PDs (Jaccard 0.731 versus expert--expert 0.751) and matched expert--expert ICC for RDA (0.371 versus 0.373).
