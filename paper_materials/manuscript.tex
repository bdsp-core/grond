...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/figS5_independent_expert_v1_irr.png}
\caption{Independent-expert v1 inter-rater reliability: pairwise expert--expert (red circles) and expert--algorithm (green squares) agreement on each task, computed under the canonical majority-accept consensus rule (a segment enters the analysis only if $\geq 2$ of 3 raters accepted it as a valid instance of the labeled pattern; per-rater labels from any rater who individually rejected a segment are also dropped). Each row shows the three pair-wise estimates; horizontal bars span the bootstrap 95\% CI envelope. The algorithm column uses V14: V12 frequency estimation (NB-Hilbert with hyperparameters retuned against the expanded consensus dataset: pass-1 bandpass 0.5--4.5~Hz; pass-2 narrowband half-width 0.5~Hz; top-3 dominant-hemisphere channels averaged; frequency search cap 4.5~Hz) plus an amplitude/rhythmicity hybrid laterality rule (default to V12's amplitude-based call; override only when four amplitude-normalized rhythmicity measures -- per-channel Q-factor, within-hemisphere phase-locking value, narrowband peak-amplitude consistency, and spectral peak prominence -- \emph{unanimously} disagree with the amplitude call). Significance of the algorithm--expert versus expert--expert difference (paired segment-bootstrap, 2000 resamples) is reported in the main text.}
\label{fig:irr_independent}
\end{figure}
