...
**Verified-empty source: `s3://bdsp-opendata-credentialed/eeg-test/eeg_bank_spec.h5`** (106 GB, 132,291 segments). Downloaded and walked exhaustively on 2026-06-07 via `code/data_management/scan_eeg_bank_for_missing_pids.py`. Walk-time 78 s. **Zero entries have a `patient_id` attribute** — the per-entry attrs are limited to `channel_names30s, eeg_duration_s, fs_hz30s, has_spectrogram, montage, note, source_dataset, source_window_center_s, spec_source`. The 200-row companion `manifest.csv` is the only patient-id mapping that exists for this bank, and it covers 11 of the 501 originally-missing PIDs (all already recovered elsewhere). None of the 8 truly-unrecoverable PD-GT patients are present. The bank cannot be used to recover them. Audit log at `results/c1_repro/eeg_bank_pid_to_seg.json` (empty dict, confirming the negative result).
