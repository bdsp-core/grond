...
# Frequency buttons at 0.25 Hz steps (0.25 to 3.5)
FREQ_BUTTONS = [round(0.25 * i, 2) for i in range(1, 15)]  # 0.25 to 3.5
...
def nvo_bandpass_grid(seg_bi):
    """Dual-band NVO: narrowband delta VE with theta nuisance regression.

    For each candidate delta frequency f, fits:
        signal ≈ a·s_delta(f) + b·s_theta + c
    where s_delta is bandpass at f ± bw and s_theta is a fixed 4-8 Hz
    bandpass. The theta component acts as a nuisance regressor, soaking
    up non-delta variance that would otherwise penalize the delta VE.

    The reported VE is the variance explained by the FULL model
    (delta + theta), but the frequency is selected based on how much
    DELTA specifically contributes (partial VE of the delta component).

    Uses sliding windows to handle intermittent RDA.
    ...
    # Pre-compute fixed theta bandpass (4-8 Hz nuisance)
    sos_theta = butter(4, [6.0, 8.0], btype='bandpass', fs=FS, output='sos')
    theta_filtered = sosfiltfilt(sos_theta, seg_bi, axis=1)
    ...
    for fi, f in enumerate(FREQ_GRID):
        delta_filtered = narrowband_filter(seg_bi, f)

        # Whole-segment VE via dual-band regression
        for ch in range(n_ch):
            sig = seg_bi[ch]
            s_d = delta_filtered[ch]
            s_t = theta_filtered[ch]
            # Least-squares: sig ≈ a·s_d + b·s_t + c
            basis = np.column_stack([s_d, s_t, np.ones(n_samp)])
            try:
                coeffs, _, _, _ = np.linalg.lstsq(basis, sig, rcond=None)
                fitted = basis @ coeffs
                residual_var = np.var(sig - fitted)
                ve_matrix[fi, ch] = max(0.0, 1.0 - residual_var / total_var[ch])
            except np.linalg.LinAlgError:
                ve_matrix[fi, ch] = 0.0

        # Sliding-window scoring: how much does DELTA add beyond theta?
        best_win_score = 0.0
        for wi in range(n_wins):
            ...
            for ch in range(n_ch):
                ...
                if seg_var < 1e-12:
                    continue
                sig_w = seg_bi[ch, s:e]
                s_d_w = delta_filtered[ch, s:e]
                s_t_w = theta_filtered[ch, s:e]
                # Partial VE: fit theta-only, then delta+theta
                # Delta's contribution = VE(delta+theta) - VE(theta-only)
                basis_full = np.column_stack([s_d_w, s_t_w, np.ones(e - s)])
                basis_theta = np.column_stack([s_t_w, np.ones(e - s)])
                try:
                    c_full, _, _, _ = np.linalg.lstsq(basis_full, sig_w, rcond=None)
                    c_theta, _, _, _ = np.linalg.lstsq(basis_theta, sig_w, rcond=None)
                    ve_full = 1.0 - np.var(sig_w - basis_full @ c_full) / seg_var
                    ve_theta = 1.0 - np.var(sig_w - basis_theta @ c_theta) / seg_var
                    # Delta's partial contribution
                    win_ve[ch] = max(0.0, ve_full - ve_theta)
                except np.linalg.LinAlgError:
                    win_ve[ch] = 0.0
...
def compute_button_filtered(seg_bi):
    """Precompute dual-band overlay for each button frequency.

    For each candidate delta freq f, fits signal ≈ a·s_delta + b·s_theta + c
    and stores the fitted combination (a·s_delta + b·s_theta) as the overlay.
    This overlay tracks the signal much better than delta-only because it
    accounts for theta contamination.

    Returns dict: freq_str -> channel dict of arrays.
    """
    from scipy.signal import sosfiltfilt
    n_ds = 400
    n_samp = seg_bi.shape[1]
    indices = np.linspace(0, n_samp - 1, n_ds).astype(int)

    # Pre-compute theta bandpass
    sos_theta = butter(4, [6.0, 8.0], btype='bandpass', fs=FS, output='sos')
    theta_filtered = sosfiltfilt(sos_theta, seg_bi, axis=1)

    result = {}
    for freq in FREQ_BUTTONS:
        delta_filtered = narrowband_filter(seg_bi, freq)
        ds = {}
        for ch in range(18):
            sig = seg_bi[ch]
            s_d = delta_filtered[ch]
            ...
            try:
                coeffs, _, _, _ = np.linalg.lstsq(basis, sig, rcond=None)
                # Overlay = delta component only (a·s_delta)
                # This shows just the RDA rhythm, with theta regressed out
                overlay = coeffs[0] * s_d
                ds[str(ch)] = overlay[indices]
            except np.linalg.LinAlgError:
                ds[str(ch)] = delta_filtered[ch, indices]
        result[str(freq)] = ds
    return result
...
def main():
    ...
    # Bandpass 0.3-5Hz for delta-focused display
    # Highpass at 0.3Hz removes baseline drift that confounds VE search
    # Lowpass at 5Hz removes theta/alpha/beta
    DISPLAY_LP = 5.0
    DISPLAY_HP = 0.3
    b_lp5, a_lp5 = butter(4, DISPLAY_LP / (FS / 2), btype='low')  # kept for compat
    ...
    for i, (_, row) in enumerate(lrda_segs_dedup.iterrows()):
        ...
        # Bandpass 0.3-5Hz for display — removes baseline drift AND high freq
        # This is critical: without removing drift, VE search picks 0.5Hz for drift
        b_bp, a_bp = butter(4, [0.3 / (FS / 2), DISPLAY_LP / (FS / 2)], btype='bandpass')
        ...
        # Run narrowband bandpass VE search on the detrended signal
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .freq-btn {{
    padding: 5px 7px; border: 1px solid #555; border-radius: 4px;
    background: #333; color: #ccc; cursor: pointer; font-family: monospace;
    font-size: 12px; font-weight: bold; min-width: 38px; text-align: center;
...
function drawEEG() {{
  ...
  if (selectedFreq && c.nb_signals) {{
    ...
    // Try multiple key formats (Python str(1.0)="1.0" but JS String(1.0)="1")
    let nbKey = String(closestBtn);
    let nbData = c.nb_signals[nbKey];
    if (!nbData) nbData = c.nb_signals[closestBtn.toFixed(1)];
    if (!nbData) nbData = c.nb_signals[closestBtn.toFixed(2)];
    if (nbData) {{
...
document.addEventListener('keydown', (e) => {{
  ...
  if (e.key === 'w' || e.key === 'W') {{
    ...
    e.preventDefault();
    if (e.shiftKey) {{
      // Shift+Left = previous case
      autoSave();
      idx = Math.max(0, idx - 1);
      show();
    }} else {{
      // Left = decrease frequency by one 0.25Hz button step
      if (selectedFreq != null) {{
        // Find closest button at or below current freq
        let bestIdx = 0;
        for (let i = 0; i < FREQ_BUTTONS.length; i++) {{
          if (FREQ_BUTTONS[i] <= selectedFreq + 0.01) bestIdx = i;
        }}
        if (bestIdx > 0) selectFreq(FREQ_BUTTONS[bestIdx - 1]);
      }} else {{
        selectFreq(FREQ_BUTTONS[0]);
      }}
    }}
  }} else if (e.key === 'ArrowRight') {{
    e.preventDefault();
    if (e.shiftKey) {{
      // Shift+Right = next case (no accept)
      autoSave();
      idx = Math.min(CASES.length - 1, idx + 1);
      show();
    }} else {{
      // Right = increase frequency by one 0.25Hz button step
      if (selectedFreq != null) {{
        // Find closest button at or above current freq
        let bestIdx = FREQ_BUTTONS.length - 1;
        for (let i = FREQ_BUTTONS.length - 1; i >= 0; i--) {{
          if (FREQ_BUTTONS[i] >= selectedFreq - 0.01) bestIdx = i;
        }}
        if (bestIdx < FREQ_BUTTONS.length - 1) selectFreq(FREQ_BUTTONS[bestIdx + 1]);
      }} else {{
        selectFreq(FREQ_BUTTONS[0]);
      }}
    }}
