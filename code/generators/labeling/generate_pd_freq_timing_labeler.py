"""
PD Frequency + Timing Labeler (LPD / GPD).

For each segment missing frequency+timing labels:
  1. Runs PDCharacterizer for default frequency/laterality prediction
  ...
  3. Generates an HTML viewer where MW can:
     ...
     - (LPD only) Label laterality (1=left, 2=right)

Usage:
    python code/generators/labeling/generate_pd_freq_timing_labeler.py --subtype lpd --batch 1 --batch-size 500
    python code/generators/labeling/generate_pd_freq_timing_labeler.py --subtype gpd --batch 1 --batch-size 500
"""
...
OUT_BASE = PROJECT_DIR / 'results' / 'labeling_tools' / 'pd_freq_timing'
...
def find_unlabeled_pd(subtype, min_votes=0):
    """Find PD segments missing frequency+timing labels, one per patient.

    A segment needs labeling if it is missing EITHER:
      - frequency: not in annotations.csv with frequency_hz AND not in
        segment_labels.csv with expert_freq_rater=MW
      - timing: mat_file's patient_id not in discharge_times.json
    ...
    pd_segs = sl_active[sl_active.subtype == subtype].copy()

    # Filter by minimum votes if specified
    if min_votes > 0:
        nv = pd.to_numeric(pd_segs.iiic_n_votes, errors='coerce').fillna(0)
        pd_segs = pd_segs[nv >= min_votes].copy()

    print(f"  Total {subtype.upper()} (>={min_votes} votes): {len(pd_segs)}")

    # --- Check which segments already have freq labels ---
    # Source 1: segment_labels.csv expert_freq_rater=MW
    has_expert_freq = pd_segs['expert_freq_rater'].fillna('') == 'MW'
    freq_labeled_pids_sl = set(pd_segs.loc[has_expert_freq, 'patient_id'].astype(str))

    # Source 2: annotations.csv with frequency_hz
    ann_path = LABELS_DIR / 'annotations.csv'
    freq_labeled_pids_ann = set()
    if ann_path.exists():
        ann = pd.read_csv(str(ann_path))
        ann_with_freq = ann[ann['frequency_hz'].notna() & (ann['frequency_hz'] > 0)]
        # Match by mat_file
        mat_files_with_freq = set(ann_with_freq['mat_file'].astype(str))
        freq_labeled_pids_ann = set(
            pd_segs.loc[pd_segs['mat_file'].astype(str).isin(mat_files_with_freq), 'patient_id'].astype(str)
        )

    freq_labeled_pids = freq_labeled_pids_sl | freq_labeled_pids_ann

    # --- Check which segments already have timing labels ---
    dt_path = LABELS_DIR / 'discharge_times.json'
    timing_labeled_pids = set()
    if dt_path.exists():
        with open(str(dt_path)) as f:
            dt = json.load(f)
        timing_labeled_pids = set(dt.keys())

    # Need labeling if missing EITHER freq or timing
    pd_segs['pid_str'] = pd_segs['patient_id'].astype(str)
    pd_segs['has_freq'] = pd_segs['pid_str'].isin(freq_labeled_pids)
    pd_segs['has_timing'] = pd_segs['pid_str'].isin(timing_labeled_pids)
    pd_segs['needs_labeling'] = ~(pd_segs['has_freq'] & pd_segs['has_timing'])

    missing = pd_segs[pd_segs['needs_labeling']].copy()
    # One segment per patient
    missing = missing.drop_duplicates(subset='patient_id')

    n_timing = len(timing_labeled_pids & set(pd_segs['pid_str']))
    print(f"  Have freq labels: {len(freq_labeled_pids)} patients")
    print(f"  Have timing labels: {n_timing} patients")
    print(f"  Missing freq OR timing: {len(missing)} patients")
    return missing


def main():
    parser = argparse.ArgumentParser(description='PD Frequency + Timing Labeler')
    parser.add_argument('--subtype', type=str, required=True, choices=['lpd', 'gpd'],
                        help='Subtype: lpd or gpd')
    ...
    for i, (_, row) in enumerate(batch.iterrows()):
        ...
        # Predict laterality/frequency
        lat_pred = predict_laterality(seg, subtype=subtype)

        # Compute evidence
        if subtype == 'gpd':
            # GPD: bilateral, use all channels
...
def build_html(cases_data, batch_num, n_batches, subtype='lpd'):
...
<head>
<meta charset="utf-8">
<title>{subtype_upper} Freq + Timing — Batch {batch_num}/{n_batches} ({n_cases} cases)</title>
<style>
...
<div id="header">
  <div id="header-left">
    <span style="font-size:16px; font-weight:bold; color:#dc143c;">{subtype_upper} Freq + Timing</span>
...
<div id="laterality-panel">
  ...
  <span id="lat-badge" class="lat-badge lat-none">{'BILATERAL' if is_gpd else '--'}</span>
  {'<span style="color:#666; font-size:12px; margin-left:8px;">N/A (GPD = bilateral)</span>' if is_gpd else '<span style="color:#666; font-size:12px; margin-left:8px;"><span class="key">1</span> Left &nbsp; <span class="key">2</span> Right</span>'}
...
<div id="shortcuts">
  {'<!-- laterality N/A for GPD -->' if is_gpd else '<span class="key">1</span> Left &nbsp;&nbsp; <span class="key">2</span> Right &nbsp;&nbsp;'}
...
const IS_GPD = {'true' if is_gpd else 'false'};
...
// Persistence
const STORAGE_KEY = '{subtype}_freq_timing_batch{batch_num}_v1';
let allLabels = {{}};
...
function channelColor(chIdx) {{
  // GPD: all channels same color (bilateral)
  if (IS_GPD) return '#000000';
  // LPD: Crimson for selected hemisphere, black for others
  if (montage === 'car') {{
...
function autoSave() {{
  ...
  allLabels[c.patient_id] = {{
    ...
    est_freq: c.est_freq,
    laterality: IS_GPD ? 'bilateral' : selectedLaterality,
...
function skipCase() {{
  ...
  markers = [];
  selectedFreq = null;
  selectedLaterality = IS_GPD ? 'bilateral' : null;
  reviewed.add(c.patient_id);
  allLabels[c.patient_id] = {{
    segment_id: c.segment_id,
    times: [],
    selected_freq: null,
    est_freq: c.est_freq,
    laterality: IS_GPD ? 'bilateral' : null,
    laterality_index: c.laterality_index,
    rejected: true,
  }};
...
function show() {{
  ...
  if (allLabels[c.patient_id] && allLabels[c.patient_id].times) {{
    ...
    selectedLaterality = IS_GPD ? 'bilateral' : c.predicted_side;
...
function exportJSON() {{
  ...
  for (const c of CASES) {{
    ...
    if (allLabels[pid]) {{
      ...
      out[pid] = {{
        ...
        rejected: isRejected,
        source: '{subtype}_freq_timing_labeler',
      }};
    }}
  }}
  ...
  a.download = '{subtype}_freq_timing_batch{batch_num}_results.json';
  a.click();
  const el = document.getElementById('save-status');
...
// Evidence canvas: click sets threshold → auto-detect peaks above it
evCanvas.addEventListener('click', function(e) {{
  ...
  // If in add/delete mode, use the old click handler (place/delete marker)
  if (mode === 'add' || mode === 'delete') {{
    handleCanvasClick(e);
    return;
  }}

  // In nav mode: click on evidence sets threshold and auto-detects peaks
  const c = CASES[idx];
  const evData = c.evidence;
  if (!evData || evData.length === 0) return;

  // Convert click Y to evidence threshold
  const threshold = Math.max(0, Math.min(1, (evBottom - y) / evH));
  if (threshold < 0.01) return;  // clicked below the trace

  // Find all peaks above threshold with minimum distance
  const nSamples = evData.length;
  const minDistSamples = Math.max(5, Math.round(nSamples / (DURATION * 4)));  // min ~0.25s apart
  ...
  if (newMarkers.length > 0) {{
    ...
    // Update frequency button to match IPI
    if (markers.length >= 2) {{
      const ipis = [];
...
evCanvas.addEventListener('mousemove', function(e) {{
  ...
  // In nav mode: draw threshold guide line on evidence
  const rect = evCanvas.getBoundingClientRect();
  const scaleY = EV_HEIGHT / rect.height;
  const y = (e.clientY - rect.top) * scaleY;
  const evTop = 10;
  ...
  // Redraw evidence with guide line
  drawEvidence();
...
document.addEventListener('keydown', (e) => {{
  if (e.key === 'a' || e.key === 'A') {{
    ...
    undo();
  }} else if (e.key === '1' && !IS_GPD) {{
    // Left laterality (LPD only)
    setLaterality('left');
  }} else if (e.key === '2' && !IS_GPD) {{
    // Right laterality (LPD only)
    setLaterality('right');
