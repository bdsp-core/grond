"""
Build annotation_viewer.html for round 3 with inlined base64 images.
"""
...
<html>
<head>
<title>Round 3 Frequency Annotation - LPD Focus</title>
<style>
...
<div id="info-panel">
  <span class="info-badge badge-lpd" id="type-badge">LPD</span>
...
try {{
  annotations = JSON.parse(localStorage.getItem('freq_annotations_round3') || '{{}}');
}} catch(e) {{ annotations = {{}}; }}

function saveAnnotations() {{
  localStorage.setItem('freq_annotations_round3', JSON.stringify(annotations));
}}
...
function updateFilterOptions() {{
  ...
  sel.options[1].text = `Unannotated (${{nUnann}})`;
  sel.options[2].text = `Annotated (${{nAnn}})`;
}}
...
function exportCSV() {{
  const headers = ['patient_id', 'file_name', 'subtype', 'f_fft', 'f_tkeo', 'f_peaks',
                   'disagreement', 'consensus_estimate', 'expert_annotation'];
  ...
  a.download = 'frequency_annotations_round3.csv';
  a.click();
}}
...
init();
    ...
    output_path = R3_DIR / 'annotation_viewer.html'
