"""
Extract RDA frequency, spatial extent, and lateralization from EEG dataset.

Runs the rda1b_fft detector (with per-channel scores) on all LRDA/GRDA segments
...
# Bipolar channel names (must match rda1b_fft.py)
BIPOLAR_CHANNELS = [
...
def main():
    ...
    for event in ['lrda', 'grda']:
        ...
        for file_path in tqdm(files_4analysis, desc=f"Processing {event.upper()}"):
            ...
            data_obj, spectra, freqs = rda.rda1b_fft(segment, fs, 0)
            ...
            # Per-channel RDA scores
            for ch in BIPOLAR_CHANNELS:
                row[f'score_{ch}'] = data_obj['channel_rda_scores'][ch]
